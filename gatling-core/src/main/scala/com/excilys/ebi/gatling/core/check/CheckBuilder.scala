 ...
 */
abstract class CheckBuilder[B <: CheckBuilder[B, T], T](what: Session => String, occurrence: Option[Int], strategy: CheckStrategy[WhatTheFuck, WhatTheFuck], expected: List[Session => String], saveAs: Option[String])
		extends Logging {

	private[gatling] def build: Check[T, WhatTheFuck, WhatTheFuck]

	private[gatling] def newInstance(what: Session => String, occurrence: Option[Int], strategy: CheckStrategy[WhatTheFuck, WhatTheFuck], expected: List[Session => String], saveAs: Option[String]): B
	private[gatling] def newInstanceWithVerify(strategy: CheckStrategy[WhatTheFuck, WhatTheFuck], expected: List[Session => String] = Nil): B with CheckBuilderSave[B]
